...
class RAGState(TypedDict):
    question: str #用户原始输入 全程不变
    query: str #当前实际检索所用的查询 初始=question
    context: str #格式化后 检索出的文本文档
    docs: List[dict] #原始文档列表
    route: Optional[str] #路由决策
    expansion_type: Optional[str] #拓展策略
    expanded_query: Optional[str] #重写后执行的查询
    step_back_question: Optional[str] #退化后生成的问题
    step_back_answer: Optional[str] #t退化后生成问题的答案
    hypothetical_doc: Optional[str] #HYDE生成的假设文档
    rag_trace: Optional[dict] #贯穿全程的追踪数据
...
def retrieve_expanded(state: RAGState) -> RAGState:
    ...
    if strategy in ("hyde", "complex"):
        ...
        auto_merge_enabled = auto_merge_enabled if auto_merge_enabled is not None else hyde_meta.get(
            "auto_merge_enabled")
        ...
        if strategy in ("step_back", "complex"):
            ...
            auto_merge_enabled = auto_merge_enabled if auto_merge_enabled is not None else step_meta.get(
                "auto_merge_enabled")
